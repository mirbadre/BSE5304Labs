...
#here make z dependent on basin size
...
writeVector(mydemw_poly, filename="mydemw.shp", filetype="ESRI Shapefile", layer="mydemw", insert=FALSE,
            overwrite=TRUE)
#bring in soils??? get list of map units
...
for(i in 1:5){
  ...
  tmpdf$sed=(tmpdf$Qpred*tmpdf$qpeak*myflowgage$area/nTIclass*100)^.56*MUSLE$KCPLSCFRG118[i]    # Eq. 4:1.1.1 SWAT Theory
...
#                 declat=myflowgage$declat,declon=myflowgage$declon)
#   tmpdf$qpeak=tmpdf$Qpred/3600/24/1000*MUSLE$areaSQKM*10^6 #m^3/sec
#   tmpdf$sed=(tmpdf$Qpred*tmpdf$qpeak*MUSLE$areaSQKM*10^6)^.56*MUSLE$KCPLSCFRG118[i]    # Eq. 4:1.1.1 SWAT Theory
#   assign(objname,tmpdf)
# }

# DF for each TI and P loss
#P loss inputs
...
#TI4
# Build a dataframe for your DP Load model in TI Class 04
...
# p loss
# Calculate your Export Coef from Easton et al 2007 Figure 2 using TI Class 5
# and note that the bold gives the TI Class. 
muTS_TI04=(((520-0)/5)*VSAsol$TIClass[2]+0) # use VSAsol$TIClass table to 
# assign TIC to calc (remember TIC05 is in location 1 in the VSAsol$TIClass 
# table
# Setting range of Soil P values (MS) using the range given
...
# assign TIC to calc (remember TIC05 is in location 1 in the VSAsol$TIClass table
# Setting range of Soil P values (MS) using the range given
...
#HW Question 1
#first find 10% of P in top 20cm of soil use MS values
P_10 <- 0.10*(MS_TI01+MS_TI02+MS_TI03+MS_TI04+MS_TI05)
#Then comment out fertilizer additions
#then create vector of total P loss from DS/LT
DPall <- DPTI01$DS + DPTI02$DS + DPTI03$DS + DPTI04$DS + DPTI05$DS
#Find time for amount
DPtot<-0
for (i in 1:length(DPLT$LT)) {
  DPtot[i] <- sum(DPLT$LT[1:i])
}
#eval at 5 yr
DPtot[1825]
yr <- (P_10/DPtot[1825])*5

#HW QUestion 2 
#Toggling between original temp value of 20 to temp of 25 to get impact on p loss
#combine 
#Then plot LT original version, then after
p2_LT <- ggplot(DPLT, aes(x=date,y=LT)) +geom_point() +theme_bw() + ggtitle('Watershed P Export at 25 degrees Celsius') +
  xlab('Date') + ylab('Phosphorus in kg/day')
p2_LT
#how do DS and Baseflow change?
p2_LB <- ggplot(DPLT, aes(x=date,y=LB)) +geom_point() +theme_bw() + ggtitle('Watershed Baseflow P Loss at 25 degrees Celsius') +
  xlab('Date') + ylab('Phosphorus in kg/day')
p2_LB
#Hw Question 3 
p3_LT <- ggplot(DPLT, aes(x=date,y=LT)) +geom_point() +theme_bw() + ggtitle('Watershed P Export at with a Fertilizer Immoblilization Rate of 2 Days') +
  xlab('Date') + ylab('Phosphorus in kg/day')
p3_LT
#rerun with tau set to 2, and look at differences
