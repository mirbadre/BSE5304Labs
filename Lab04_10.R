...
# giving at least 9 (3*3) areas (HRUs). 
#
...
#
###########################################################################################
#Here we will create a separate MUSLE df for pusle values determined by TI slope called MUSLE_TI
MUSLE_TI <- MUSLE
#Get PUSLE values by TI classes
MUSLE_TI$pusle=0 #initiate our vector
for (i in 1:length(MUSLE_TI$pusle)) {
  if (MUSLE_TI$slp[i] < 0.02) {
    MUSLE_TI$pusle[i] <- 0.6
  } else if (MUSLE_TI$slp[i] < 0.08) {
    MUSLE_TI$pusle[i] <- 0.5
  } else if (MUSLE_TI$slp[i] < 0.12) {
    MUSLE_TI$pusle[i] <- 0.6
  } else if (MUSLE_TI$slp[i] < 0.16) {
    MUSLE_TI$pusle[i] <- 0.7
  } else if (MUSLE_TI$slp[i] < 0.20) {
    MUSLE_TI$pusle[i] <- 0.8
  } else {
    MUSLE_TI$pusle[i] <- 0.9
  }
}
################################################
#Now back to one pusle value across wshed
# Pusle
MUSLE$Pusle=.50
#
# Cusle
...
attach(MUSLE)
MUSLE$KCPLSCFRG118=11.8*0.28*Cusle*Pusle*LS*CFRG
detach(MUSLE)
MUSLE # Make sure values look correct, Pusle, Cusle, Kusle
#MUSLE VALUES FOR TI with different pusle
attach(MUSLE_TI)
MUSLE_TI$KCPLSCFRG118=11.8*0.28*Cusle*pusle*LS*CFRG
detach(MUSLE_TI)
MUSLE_TI
#
...
VSAParams_TI=merge(VSAsol,MUSLE_TI,by.x="TIClass",by.y="TIclass")
View(VSAParams)
...
# For TIC01 without adapted pusle TIC01 with pusle TIC01_p
detach(BasinTMWB_JO)
detach(CNmodeldf)
TIC01 = CNmodel(CNmodeldf = TIC01, CNavg=VSAParams$CN[1], 
                  declat=myflowgage$declat,declon=myflowgage$declon)
TIC01_p = CNmodel(CNmodeldf = TIC01_p, CNavg=VSAParams_TI$CN[1], 
                declat=myflowgage$declat,declon=myflowgage$declon)
...
TIC01_p$qpeak=TIC01_p$Qpred/3600/24/1000*myflowgage$area/nTIclass*10^6 #m^3/sec
TIC01_p$sed=(TIC01_p$Qpred*TIC01_p$qpeak*myflowgage$area/nTIclass*100)^.56*MUSLE_TI$KCPLSCFRG118[1] 
# Route the water and continue
TIC02$P=TIC01$Qpred+TIC02$P
TIC02_p$P=TIC02$P

#My way of getting these values for TC2-TC5
TIC02 = CNmodel(CNmodeldf = TIC02, CNavg=VSAParams$CN[2], 
                declat=myflowgage$declat,declon=myflowgage$declon)
TIC02_p = CNmodel(CNmodeldf = TIC02_p, CNavg=VSAParams_TI$CN[2], 
                  declat=myflowgage$declat,declon=myflowgage$declon)
...
TIC02_p$qpeak=TIC02_p$Qpred/3600/24/1000*myflowgage$area/nTIclass*10^6 #m^3/sec
TIC02_p$sed=(TIC02_p$Qpred*TIC02_p$qpeak*myflowgage$area/nTIclass*100)^.56*MUSLE_TI$KCPLSCFRG118[2]

#Next TI, route water, get df and qpeak and sed
TIC03$P=TIC02$Qpred+TIC03$P
TIC03_p$P=TIC03$P
TIC03 = CNmodel(CNmodeldf = TIC03, CNavg=VSAParams$CN[3], 
                declat=myflowgage$declat,declon=myflowgage$declon)
TIC03_p = CNmodel(CNmodeldf = TIC03_p, CNavg=VSAParams_TI$CN[3], 
                  declat=myflowgage$declat,declon=myflowgage$declon)
...
TIC03_p$qpeak=TIC03_p$Qpred/3600/24/1000*myflowgage$area/nTIclass*10^6 #m^3/sec
TIC03_p$sed=(TIC03_p$Qpred*TIC03_p$qpeak*myflowgage$area/nTIclass*100)^.56*MUSLE_TI$KCPLSCFRG118[3]
...
TIC04 = CNmodel(CNmodeldf = TIC04, CNavg=VSAParams$CN[4], 
                declat=myflowgage$declat,declon=myflowgage$declon)
TIC04_p = CNmodel(CNmodeldf = TIC04_p, CNavg=VSAParams_TI$CN[4], 
                  declat=myflowgage$declat,declon=myflowgage$declon)
...
TIC04_p$qpeak=TIC04_p$Qpred/3600/24/1000*myflowgage$area/nTIclass*10^6 #m^3/sec
TIC04_p$sed=(TIC04_p$Qpred*TIC04_p$qpeak*myflowgage$area/nTIclass*100)^.56*MUSLE_TI$KCPLSCFRG118[4]
#Next TI, route water, get df and qpeak and sed
...
TIC05 = CNmodel(CNmodeldf = TIC05, CNavg=VSAParams$CN[5], 
                declat=myflowgage$declat,declon=myflowgage$declon)
TIC05_p = CNmodel(CNmodeldf = TIC05_p, CNavg=VSAParams_TI$CN[5], 
                  declat=myflowgage$declat,declon=myflowgage$declon)
...
TIC05_p$qpeak=TIC05_p$Qpred/3600/24/1000*myflowgage$area/nTIclass*10^6 #m^3/sec
TIC05_p$sed=(TIC05_p$Qpred*TIC05_p$qpeak*myflowgage$area/nTIclass*100)^.56*MUSLE_TI$KCPLSCFRG118[5]

#Plot time series loss of sediment and total agreggate for each TI, with original values
plot1 <- ggplot(TIC01, aes(x=date,y=sed)) +geom_point() +theme_bw() + ggtitle('Sediment Loss Across Time in TI Class 1') +
  xlab('Date') + ylab('Sediment')
plot1

plot2 <- ggplot(TIC02, aes(x=date,y=sed)) +geom_point() +theme_bw() + ggtitle('Sediment Loss Across Time in TI Class 2') +
  xlab('Date') + ylab('Sediment')
plot2

plot3 <- ggplot(TIC03, aes(x=date,y=sed)) +geom_point() +theme_bw() + ggtitle('Sediment Loss Across Time in TI Class 3') +
  xlab('Date') + ylab('Sediment')
plot3

plot4 <- ggplot(TIC04, aes(x=date,y=sed)) +geom_point() +theme_bw() + ggtitle('Sediment Loss Across Time in TI Class 4') +
  xlab('Date') + ylab('Sediment')
plot4

plot5 <- ggplot(TIC05, aes(x=date,y=sed)) +geom_point() +theme_bw() + ggtitle('Sediment Loss Across Time in TI Class 5') +
  xlab('Date') + ylab('Sediment')
plot5
...
plot_agg <- ggplot() + geom_point(data=TIC01_p, aes(x=date,y=cumsum(sed), color='TI 1')) + 
  ...
  geom_point(data=TIC03_p, aes(x=date,y=cumsum(sed), color='TI 3')) + 
  geom_point(data=TIC04_p, aes(x=date,y=cumsum(sed), color='TI 4')) + 
  geom_point(data=TIC05_p, aes(x=date,y=cumsum(sed), color='TI 5 (total)')) + 
  ggtitle('Total Aggregate Loss Across Time by TI Class') + theme_bw() + 
  xlab('Date') + ylab('Total Aggregate (metric tons)')
plot_agg
  

#Plot TI adjusted Pusle, and non-adjusted PUSLE in same plot
plotall1 <- ggplot() + geom_point(data=TIC01, aes(x=date,y=sed, color='Original Pusle Values')) + 
  geom_point(data=TIC01_p, aes(x=date,y=sed, color='Updated Pusle Values by TI Slope')) + theme_bw() +
  theme(legend.position = 'bottom', legend.justification = 'center',
        legend.box.just='left', legend.background=element_blank()) +
  ggtitle('Sediment Loss Across Time in TI Class 1') + 
  labs(x ='Date',y="Sediment (metric tons)", color='Legend')
plotall1

plotall2 <- ggplot() + geom_point(data=TIC02, aes(x=date,y=sed, color='Original Pusle Values')) + 
  geom_point(data=TIC02_p, aes(x=date,y=sed, color='Updated Pusle Values by TI Slope')) + theme_bw() +
  theme(legend.position = 'bottom', legend.justification = 'center',
        legend.box.just='left', legend.background=element_blank()) +
  ggtitle('Sediment Loss Across Time in TI Class 2') + 
  labs(x ='Date',y="Sediment (metric tons)", color='Legend')
plotall2

plotall3 <- ggplot() + geom_point(data=TIC03, aes(x=date,y=sed, color='Original Pusle Values')) + 
  geom_point(data=TIC03_p, aes(x=date,y=sed, color='Updated Pusle Values by TI Slope')) + theme_bw() +
  theme(legend.position = 'bottom', legend.justification = 'center',
        legend.box.just='left', legend.background=element_blank()) +
  ggtitle('Sediment Loss Across Time in TI Class 3') + 
  labs(x ='Date',y="Sediment (metric tons)", color='Legend')
plotall3

plotall4 <- ggplot() + geom_point(data=TIC04, aes(x=date,y=sed, color='Original Pusle Values')) + 
  geom_point(data=TIC04_p, aes(x=date,y=sed, color='Updated Pusle Values by TI Slope')) + theme_bw() +
  theme(legend.position = 'bottom', legend.justification = 'center',
        legend.box.just='left', legend.background=element_blank()) +
  ggtitle('Sediment Loss Across Time in TI Class 4') + 
  labs(x ='Date',y="Sediment (metric tons)", color='Legend')
plotall4

plotall5 <- ggplot() + geom_point(data=TIC05, aes(x=date,y=sed, color='Original Pusle Values')) + 
  geom_point(data=TIC05_p, aes(x=date,y=sed, color='Updated Pusle Values by TI Slope')) + theme_bw() +
  theme(legend.position = 'bottom', legend.justification = 'center',
        legend.box.just='left', legend.background=element_blank()) +
  ggtitle('Sediment Loss Across Time in TI Class 5') + 
  labs(x ='Date',y="Sediment (metric tons)", color='Legend')
plotall5
