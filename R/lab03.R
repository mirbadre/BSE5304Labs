...
#Download soils data
url="https://websoilsurvey.sc.egov.usda.gov/DSD/Download/AOI/o3drwg1m3nmzdofcevax14cz/wss_aoi_2023-02-09_13-08-39.zip"
...
mysoil=readOGR("wss_aoi_2023-02-09_13-08-39/spatial/soilmu_a_aoi.shp")    
...
#Fill in the code for the other two functions:
...
#HERE HAVING ISSUE!! cant get AW column in df to show up
attach(TMWB)
for (t in 2:length(date)){
  if (dP[t]< 0) {  
...
#TMWB$AW = 0 # AW already intialized??
#TMWB$Excess = 0 # isn't this also already initialized??
...
#Homework 1, plot the Precipitation, ET, AW, Excess, Qpred, and Qmm with the ggplot library from last week
attach(TMWB)
HW1plot <- ggplot(TMWB, aes(x=date)) +
  geom_line( aes(y=P, color= 'Precip'), linewidth=0.8) +
  geom_line( aes(y=PET, color= 'ET'), linewidth=1) +
  geom_line( aes(y=Excess, color= 'Excess'), linewidth=0.8) +
  geom_line( aes(y=Qpred, color= 'Qpred'), linewidth=0.8) +
  geom_line( aes(y=Qmm, color= 'Qmm'), linewidth=0.8) +
  theme(legend.position = 'bottom', legend.justification = 'center',
        legend.box.just='left', legend.background=element_blank()) + 
  ggtitle(myflowgage$gagename) +
  labs(x ='Date', color='Legend') + theme_bw() +
  scale_y_continuous(name = "Water Depth",sec.axis = sec_axis(~.*3, name="Available Water")) +
  geom_line( aes(y=AW/3, color= 'AW'), linewidth=0.8) 
HW1plot
#Having issues with pdf function will manually print to pdf
detach(TMWB)
