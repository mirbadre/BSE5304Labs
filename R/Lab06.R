...
f <- function (x) {
  ...
  return(mNSE,CNmodelnew)
}
#            Educated guesses for ranges… or not.        
#            CN,  Ia, Slp, Asp, AWC,Depth,ResCoef
lower <- c(30,0.05, 0.01, 0.0, 0.2,  500,   0.1)
upper <- c(99,0.20, 0.05, 0.1, 0.3, 2000,   0.5)
...
dat=data.frame(Qm3ps=CN_DF$Qmm,
                 Jday=strptime(CN_DF$date,"%Y-%m-%d")$yday+1,                 Precip=CN_DF$P,TMX=CN_DF$MaxTemp,TMN=CN_DF$MinTemp,
...
set.seed(2)
# setup and train the neural network, toggle with removing Qm3ps_lag for HW q3
...
plot(CN_DF$date, CN_DF$Qmm,type="l",col="black")
...
lines(CN_DF$date, CN_DF$Qpred,type="l",col="blue")
...
comparedf=merge(CN_DF,NNFlowModel)
